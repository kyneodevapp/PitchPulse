...
export function MatchCard({
    ...
    return (
        <>
            {/* Single motion.div for card entry only — all internal animations use CSS */}
            <motion.div
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.25, ease: "easeOut" }}
                className={cn(
                    "relative bg-[#0D1117] rounded-xl overflow-hidden border",
                    "flex flex-col h-full cursor-pointer shadow-lg",
            ...
            >
                {/* Tier accent left bar */}
                <div className={cn("absolute left-0 top-0 bottom-0 w-[3px] z-10", style.leftBar)} />

                <div className="pl-5 pr-5 pt-4 pb-5 flex-1 flex flex-col">

                    {/* Row 1: League pill + badges */}
                                    ...
                                    {lockTip && (
                                        <div className="absolute bottom-full right-0 mb-2 w-44 bg-[#0D1117] border border-[#1F2937] rounded-lg p-2.5 shadow-xl z-50">
                                            <p className="text-[9px] font-medium text-neutral-300 leading-relaxed">
                                                Prediction locked — result tracked in History
                                            </p>
                                        </div>
                                    )}
                                </div>
                    ...
                    <div className="flex justify-end mb-4">
                        <div className={cn(
                            ...
                            isLive
                                ...
                                : "bg-[#161B22] text-neutral-500 border border-[#1E293B]"
                        )}>
                            {isLive ? (
                    ...
                    <div className="flex items-end justify-between mb-5">
                        ...
                        <div className="flex flex-col items-end flex-shrink-0">
                            ...
                            <span className={cn("text-3xl font-black tabular-nums leading-none", style.text)}>
                                {displayOdds ? displayOdds.toFixed(2) : "—"}
                            </span>
                        </div>
                    </div>

                    {/* Metrics Grid — pure CSS, no Framer Motion */}
                    <div className="mt-auto space-y-2">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            ...
                            <MetricCell label="Edge" value={`+${(displayEdge * 100).toFixed(1)}%`} highlight accentClass={style.metricAccent} />
                        </div>

                        {/* Row 2 */}
                        <div className="grid grid-cols-3 gap-2">
                            <MetricCell label="EV" value={`+${(displayEV * 100).toFixed(1)}%`} highlight accentClass={style.metricAccent} />
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            <div className={cn("flex flex-col items-center p-2 rounded-lg border", style.riskBg)}>
                            ...
                            {/* Confidence with CSS-animated bar */}
                        ...
                        {/* Full Analysis Button — CSS hover only */}
                        <div className="group/btn mt-1 w-full py-3 rounded-lg text-[11px] font-bold text-white/60 hover:text-white transition-colors flex items-center justify-center gap-2 border border-white/10 hover:border-white/20 hover:bg-white/5">
                            Full Analysis
...
function MetricCell({ label, value, highlight, accentClass }: {
    label: string;
    value: string;
    highlight?: boolean;
    accentClass?: string;
    ...
    return (
        <div className={cn(
            ...
            <span className={cn("text-sm font-bold tabular-nums", highlight ? "text-emerald-400" : "text-white/90")}>
                {value}
            </span>
        </div>
    );
